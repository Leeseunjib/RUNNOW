...
## VIP 전용 서버 AI 코치 (chatWithCoach)

PRO는 사용자 본인의 구글 AI 키(BYOK)를 씁니다. VIP만 서버가 키를 대신 쓰므로
사용자가 아무것도 설정하지 않아도 됩니다.
...
| 적용 상한 | **하루 100회 / 월 1,500회** |
| 상한 적용 시 월 비용 | $3.26 → **마진 82%** |
| 2027년 단가 2배 인상 후 | $6.52 → **마진 63%** |

일반적인 헤비 유저도 하루 20~50회 수준이라 상한이 체감되지 않습니다.
상한은 `functions/aiCoach.js`의 `DAILY_CALL_LIMIT` 한 곳에서 조정합니다.
바꾸면 `tests/aiCoach.test.mjs`가 손익분기를 자동으로 재검증하고,
`tests/paywall.test.mjs`가 판매 문구의 상한 표기와 일치하는지 확인합니다.
...
|---|---|
| 로그인 여부 | 비로그인 무단 호출 |
| 구독 문서 직접 조회 | 클라이언트가 VIP라고 주장 |
| 만료일 확인 | 만료된 구독으로 계속 사용 |
| PRO 거부 | 저가 요금제로 서버 AI 사용 |
| 트랜잭션 기반 차감 | 동시 호출로 상한 우회 |
| 입력 길이 제한 | 긴 프롬프트로 토큰 폭증 |
| 실패 시 차감 롤백 | 서버 오류인데 사용자 한도 소모 |

`ai_usage` 컬렉션은 `firestore.rules`에서 클라이언트 쓰기를 차단했습니다.
클라이언트가 고칠 수 있으면 상한이 무의미해집니다.
